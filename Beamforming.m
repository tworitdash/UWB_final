...
fmin = f(fminn);
fmax = f(fmaxn);
disp(strcat('Operational Bandwidth, Start Frequency: ',num2str(fmin./1e9),'GHz, End frequency: ', num2str(fmax./1e9),'GHz'));
f = f(fminn:fmaxn);
Nfre = length(f);
...
figure;
plot(X,Projection_dB,'k','Linewidth',2, 'color', [0.6350, 0.0780, 0.1840]);
xlim(focX);
grid on;
xlabel('X [m]');ylabel('Beam pattern [dB]');box on;
title(strcat('Center frequency:', num2str(fc./1e9),'GHz, Bandwidth:', num2str(B./1e9),'GHz, Sampling step:', num2str(Sfactor),'cm'));
...
% 
% ind2 = interp1(temp,1:length(temp),-3, 'linear');
% y2 = f(ind2);
% 
% dely = y2 - y1

figure;
plot(Y,Projection_dB_2,'k','Linewidth',2, 'color', [0.6350, 0.0780, 0.1840]);
xlim(focY);
grid on;
